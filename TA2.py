...
while carryOn:
 ...
 #update location here 
  ...
  p1=firebase.get("","/player1")
  p2=firebase.get("","/player2")
  p3=firebase.get("","/player3")
  time_dict={p1["time"]:p1["name"],
            p2["time"]:p2["name"],
            p3["time"]:p3["name"]}
  min_time=min(time_dict)
  winner=time_dict[min_time]
  ...
  if counter>=5 and cary<=50:
    ...
    text = font.render("Winner:"+winner+":"+str(min_time), 1,(255,100,0))
    screen.blit(text, (50,200))
    pygame.display.flip()
    pygame.time.wait(2000)
    break
